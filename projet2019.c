...
void* ld_delete_node(void* liste, void* n){
    ...
    node* next_node;
    if(n==NULL) return NULL;
    if(ld_previous(hd,curr)!=NULL) prev_node =(node*) ld_previous(hd,curr);
    
    if(ld_next(hd,curr)!= NULL) next_node =(node*) ld_next(hd,curr);
    ...
    if(!is_merged){
        if(hd->tab_tranches_size-1 <= hd->nb_elem_tab_tranches){  //tableau de tranches remplie
